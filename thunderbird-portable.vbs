'Registers Thunderbird Portable with Default Programs or Default Apps in Windows
'thunderbirdportable.vbs - created by Ramesh Srinivasan for Winhelponline.com
'v1.0 23-July-2022 - Initial release. Tested on Mozilla Thunderbird 102.0.3.
...
'Quit if ThunderbirdPortable.exe is missing in the current folder!
If Not oFSO.FileExists (sAppPath & "\ThunderbirdPortable.exe") Then
   MsgBox "Please run this script from Thunderbird Portable folder. The script will now quit.", _
   vbOKOnly + vbInformation, "Register Thunderbird Portable with Default Apps"
   WScript.Quit
End If
...
If intOSBitness = 64 Then
   sIconPath = sAppPath & "\App\Thunderbird64\thunderbird.exe"
   sDLLPath = sAppPath & "\App\Thunderbird64\"
ElseIf intOSBitness = 32 Then
   sIconPath = sAppPath & "\App\Thunderbird\thunderbird.exe"
   sDLLPath = sAppPath & "\App\Thunderbird\"
Else
   WScript.Quit
End If

sExecPath = sAppPath & "\ThunderbirdPortable.exe"
...
sMailKey = sbaseKey & "Clients\Mail\Mozilla Thunderbird Portable\"
sNewsKey = sbaseKey & "Clients\News\Mozilla Thunderbird Portable\"
sCalKey = sbaseKey & "Clients\Calendar\Mozilla Thunderbird Portable\"
...
Else
   sAction = InputBox ("Type REGISTER to add Thunderbird Portable to Default Apps. Type UNREGISTER to remove.", _
   "Thunderbird Portable Registration", "REGISTER")
...
Sub RegisterThunderbirdPortable(sExecPath)
   ...
   '----------------------------------------------------------------
   WshShell.RegWrite sbaseKey & "RegisteredApplications\Thunderbird Portable", _
   "Software\Clients\Mail\Mozilla Thunderbird Portable\Capabilities", "REG_SZ"
   
   WshShell.RegWrite sbaseKey & "RegisteredApplications\Thunderbird Portable (News)", _
   "Software\Clients\News\Mozilla Thunderbird Portable\Capabilities", "REG_SZ"
   
   WshShell.RegWrite sbaseKey & "RegisteredApplications\Thunderbird Portable (Calendar)", _
   "Software\Clients\Calendar\Mozilla Thunderbird Portable\Capabilities", "REG_SZ"
   ...
   '----------------------------------------------------------------
   WshShell.RegWrite sbaseKey & "Clients\Mail\", "Mozilla Thunderbird Portable", "REG_SZ"
   WshShell.RegWrite sMailKey, "Mozilla Thunderbird Portable", "REG_SZ"
   ...
   WshShell.RegWrite sMailKey & "Capabilities\ApplicationName", "Thunderbird Portable", "REG_SZ"
   ...
   WshShell.RegWrite sMailKey & "Capabilities\StartMenu\Mail", "Mozilla Thunderbird Portable", "REG_SZ"
   ...
   '----------------------------------------------------------------
   WshShell.RegWrite sNewsKey, "Mozilla Thunderbird Portable", "REG_SZ"
   ...
   WshShell.RegWrite sNewsKey & "Capabilities\ApplicationName", "Thunderbird Portable (News)", "REG_SZ"
   ...
   For Each regkey In ArrKeys
      WshShell.RegWrite regkey, "Mozilla Thunderbird Portable", "REG_SZ"
      WshShell.RegWrite regkey & "DefaultIcon", sIconPath & ",0", "REG_SZ"
      WshShell.RegWrite regkey & "DLLPath", sDLLPath & "mozMapi32.dll", "REG_SZ"
   Next
   
   WshShell.RegWrite sCalKey, "Mozilla Thunderbird Portable", "REG_SZ"
   ...
   WshShell.RegWrite sCalKey & "Capabilities\ApplicationName", "Thunderbird Portable", "REG_SZ"  
   ...
   For Each regkey In ArrKeys
      WshShell.RegWrite sCalKey & "Protocols\" & regkey & "\", "Thunderbird Portable (Calendar) URL", "REG_SZ"
      WshShell.RegWrite sCalKey & "Protocols\" & regkey & "\FriendlyTypeName", "Thunderbird Portable (Calendar) URL", "REG_SZ"
   ...
   WshShell.RegWrite sClassesKey & "MozillaMapi\", "Mozilla MAPI", "REG_SZ"
   ...
   WshShell.RegWrite sClassesKey & "MozillaMapi.1\", "Mozilla MAPI", "REG_SZ"
   ...
   '(i.e., -- "Mozilla Thunderbird, Portable Edition" Vs. "Thunderbird Portable")
   'The official Mozilla Thunderbird setup doesn't add this registry key.
   WshShell.RegWrite sClassesKey & "Thunderbird.Url.mailto2\Application\ApplicationIcon", sIconPath & ",0", "REG_SZ"
   WshShell.RegWrite sClassesKey & "Thunderbird.Url.mailto2\Application\ApplicationName", "Thunderbird Portable", "REG_SZ"
   
   'Launch Default Programs or Default Apps after registering Thunderbird Portable   
...
Sub UnRegisterThunderbirdPortable
   ...
   tmpfile.Writeline Chr(34) & "Thunderbird Portable" & Chr(34) & "=-"
   tmpfile.Writeline Chr(34) & "Thunderbird Portable (Calendar)" & Chr(34) & "=-"
   tmpfile.Writeline Chr(34) & "Thunderbird Portable (News)" & Chr(34) & "=-"
   tmpfile.writeblanklines 1
   tmpfile.Writeline "[" & sbaseKey & "Clients\Mail" & "]"
   tmpfile.Writeline "@=-"
   tmpfile.writeblanklines 1
   
   ArrKeys = Array ("Clients\Mail\Mozilla Thunderbird Portable", _
   "Clients\News\Mozilla Thunderbird Portable", _
   "Clients\Calendar\Mozilla Thunderbird Portable", _
   ...
   'Launch Default Apps after unregistering Thunderbird Portable
   WshShell.Run "control /name Microsoft.DefaultPrograms /page pageDefaultProgram"   
End Sub
